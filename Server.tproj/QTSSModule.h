...
public:
	...
	//
	// MODIFIERS
	void SetPrefsDict(QTSSPrefs* inPrefs) {
		fPrefs = inPrefs;
	}
	...
	OSQueueElem* GetQueueElem() {
		return &fQueueElem;
	}
	
	Bool16 IsInitialized() {
		return fDispatchFunc != NULL;
	}
	
	QTSSPrefs* GetPrefsDict() {
		return fPrefs;
	}
	
	QTSSDictionary* GetAttributesDict() {
		return fAttributes;
	}
	
	OSMutex* GetAttributesMutex() {
		return &fAttributesMutex;
	}
	...
	QTSS_Error CallDispatch(QTSS_Role inRole, QTSS_RoleParamPtr inParams) {
		...
		if (MODULE_DEBUG) {
			this->GetValue(qtssModName)->PrintStr(
					"QTSSModule::CallDispatch ENTER module=", " role=");
		...
		if (MODULE_DEBUG) {
			this->GetValue(qtssModName)->PrintStr(
					"QTSSModule::CallDispatch EXIT  module=", " role=");
			if (theRoleIndex != -1)
				qtss_printf(" %s EXIT\n", sRoleNames[theRoleIndex]);
		}
	...
	QTSS_ModuleState* GetModuleState() {
		return &fModuleState;
	}
