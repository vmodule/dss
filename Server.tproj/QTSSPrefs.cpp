...
QTSSPrefs::QTSSPrefs(
	XMLPrefsParser* inPrefsSource, StrPtrLen* inModuleName,
	QTSSDictionaryMap* inMap, Bool16 areInstanceAttrsAllowed,
	QTSSPrefs* parentDictionary) 
...
QTSSDictionary* QTSSPrefs::CreateNewDictionary(QTSSDictionaryMap* inMap,
		OSMutex* /* inMutex */)
{
...
void QTSSPrefs::RereadObjectPreferences(ContainerRef container)
{
	...
	{
		modulePrefInServer = NEW
		char*[initialNumAttrs ];
	...
	for (UInt32 i = 0; i < initialNumAttrs; i++) // pull out all the names in the server
	{
		...
		theErr = this->GetInstanceDictMap()->GetAttrInfoByIndex(i,
				&theAttrInfoPtr);
		if (theErr != QTSS_NoErr)
			continue;

		UInt32 nameLen = 0;
		theErr = theAttrInfoPtr->GetValuePtr(qtssAttrName, 0,
				(void **) &modulePrefInServer[i], &nameLen);
	...
	for (UInt32 x = 0; x < theNumPrefs; x++)
	{
		...
		(void) fPrefsSource->GetPrefValueByIndex(container, x, 0, &thePrefName,
				&thePrefTypeStr);

		// What type is this data type?
		QTSS_AttrDataType thePrefType = QTSSDataConverter::TypeStringToType(
				thePrefTypeStr);
		...
		if (this->GetInstanceDictMap() != NULL)
			(void) this->GetInstanceDictMap()->GetAttrInfoByName(thePrefName,
					&theAttrInfo, false); // false=don't return info on deleted attributes
		...
		{
			if (modulePrefInServer[i] != NULL && thePrefName != NULL
					&& 0 == ::strcmp(modulePrefInServer[i], thePrefName))
			{
		...
		else
		{
			...
			theErr = theAttrInfo->GetValue(qtssAttrDataType, 0, &theAttrType,
					&theLen);
			Assert(theErr == QTSS_NoErr);
			...
			SInt32 theIndex =
					this->GetInstanceDictMap()->ConvertAttrIDToArrayIndex(
							theAttrID);
			Assert(theIndex >= 0);
		}
	}
	...
	if (this->GetInstanceDictMap() != NULL && initialNumAttrs > 0)
	{
		for (UInt32 a = 0; a < initialNumAttrs; a++)
		{
			...
			{
				QTSSAttrInfoDict* theAttrInfoPtr = NULL;
				theErr = this->GetInstanceDictMap()->GetAttrInfoByName(
						modulePrefInServer[a], &theAttrInfoPtr);
				Assert(theErr == QTSS_NoErr);
				if (theErr != QTSS_NoErr)
					continue;
				...
				theErr = theAttrInfoPtr->GetValue(qtssAttrID, 0, &theAttrID,
						&theLen);
				Assert(theErr == QTSS_NoErr);
				if (theErr != QTSS_NoErr)
					continue;
				...
				if (0)
				{
					char* theName = NULL;
					UInt32 nameLen = 0;
					theAttrInfoPtr->GetValuePtr(qtssAttrName, 0,
							(void **) &theName, &nameLen);
					qtss_printf(
...
void QTSSPrefs::SetPrefValuesFromFile(ContainerRef container,
		UInt32 inPrefIndex, QTSS_AttributeID inAttrID, UInt32 inNumValues)
{
		...
		QTSS_AttributeID inAttrID, UInt32 inNumValues)
{
	...
	for (UInt32 y = 0; y < numPrefValues; y++)
	{
		...
		thePrefValue = fPrefsSource->GetPrefValueByRef(pref, y, &thePrefName,
				&thePrefTypeStr);
	...
	{
		thePrefValue = fPrefsSource->GetPrefValueByRef(pref, z, &thePrefName,
				&thePrefTypeStr);
		this->SetPrefValue(inAttrID, z, thePrefValue, thePrefType,
				maxPrefValueSize);
	}
...
void QTSSPrefs::SetObjectValuesFromFile(ContainerRef pref,
		QTSS_AttributeID inAttrID, UInt32 inNumValues, char* prefName)
{
	for (UInt32 z = 0; z < inNumValues; z++)
	{
		...
		{
			UInt32 tempIndex;
			err = CreateObjectValue(inAttrID, &tempIndex,
					(QTSSDictionary**) &prefObject, NULL,
...

{
	...
	QTSS_Error theErr = QTSSDataConverter::StringToValue(inPrefValue,
			inPrefType, convertedPrefValue, &convertedBufSize);
	Assert(theErr == QTSS_NoErr);
...
QTSS_AttributeID QTSSPrefs::AddPrefAttribute(const char* inAttrName,
		QTSS_AttrDataType inDataType)
{
	QTSS_Error theErr = this->AddInstanceAttribute(inAttrName, NULL, inDataType,
			qtssAttrModeRead | qtssAttrModeWrite | qtssAttrModeDelete);
	Assert(theErr == QTSS_NoErr);
...
void QTSSPrefs::RemoveValueComplete(UInt32 inAttrIndex,
		QTSSDictionaryMap* inMap, UInt32 inValueIndex)
{
	ContainerRef objectRef = GetContainerRef();
	ContainerRef pref = fPrefsSource->GetPrefRefByName(objectRef,
			inMap->GetAttrName(inAttrIndex));
	Assert(pref != NULL);
	if (pref != NULL)
		fPrefsSource->RemovePrefValue(pref, inValueIndex);

	if (fPrefsSource->WritePrefsFile())
		QTSSModuleUtils::LogError(qtssWarningVerbosity, qtssMsgCantWriteFile,
				0);
}

void QTSSPrefs::RemoveInstanceAttrComplete(UInt32 inAttrIndex,
...
{
	ContainerRef objectRef = GetContainerRef();
	ContainerRef pref = fPrefsSource->AddPref(objectRef,
			inMap->GetAttrName(inAttrIndex),
			QTSSDataConverter::TypeToTypeString(
					inMap->GetAttrType(inAttrIndex)));
	...
	{
		OSCharArrayDeleter theValueAsString(
				QTSSDataConverter::ValueToString(inNewValue, inNewValueLen,
						inMap->GetAttrType(inAttrIndex)));
		fPrefsSource->SetPrefValue(pref, inValueIndex,
				theValueAsString.GetObject());
	}
...
ContainerRef QTSSPrefs::GetContainerRefForObject(QTSSPrefs* object)
{
	...
	ContainerRef pref = fPrefsSource->GetPrefRefByName(thisContainer,
			object->fPrefName);
	if (pref == NULL)
		return NULL;
	...
	QTSSAttrInfoDict* theAttrInfoPtr = NULL;
	QTSS_Error theErr = this->GetInstanceDictMap()->GetAttrInfoByName(
			object->fPrefName, &theAttrInfoPtr);
	Assert(theErr == QTSS_NoErr);
	if (theErr != QTSS_NoErr)
		return NULL;
	QTSS_AttributeID theAttrID = qtssIllegalAttrID;
