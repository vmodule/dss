...
public:
	...
	enum
	{
		kNoFlags = 0, kDontObeyReadOnly = 1, kDontCallCompletionRoutine = 2
	};
	...
	QTSS_Error GetValue(QTSS_AttributeID inAttrID, UInt32 inIndex,
	...
	{
		return GetValuePtr(inAttrID, inIndex, outValueBuffer, outValueLen,
				false);
	}
	...
	QTSS_Error GetValueAsString(QTSS_AttributeID inAttrID, UInt32 inIndex,
			char** outString);
	...
	// Returns:     QTSS_BadArgument, QTSS_ReadOnly (if attribute is read only),
	QTSS_Error SetValuePtr(QTSS_AttributeID inAttrID, const void* inBuffer,
			UInt32 inLen, UInt32 inFlags = kNoFlags);
	...
	QTSS_Error RemoveValue(QTSS_AttributeID inAttrID, UInt32 inIndex,
			UInt32 inFlags = kNoFlags);

	// Utility routine used by the two external flavors of GetValue
	QTSS_Error GetValuePtr(QTSS_AttributeID inAttrID, UInt32 inIndex,
			void** outValueBuffer, UInt32* outValueLen, Bool16 isInternal);

	//
	// ACCESSORS

	QTSSDictionaryMap* GetDictionaryMap()
	{
		return fMap;
	}
	...
	QTSSDictionaryMap* GetInstanceDictMap()
	{
		return fInstanceMap;
	}
	...
	void SetLocked(Bool16 inLocked)
	{
		fLocked = inLocked;
	}
	...
	QTSS_Error GetAttrInfoByName(const char* inAttrName,
			QTSSAttrInfoDict** outAttrInfoDict);
	QTSS_Error GetAttrInfoByID(QTSS_AttributeID inAttrID,
			QTSSAttrInfoDict** outAttrInfoDict);
	...
	void SetVal(QTSS_AttributeID inAttrID, void* inValueBuffer,
			UInt32 inBufferLen);
...
protected:
	...
	virtual void RemoveValueComplete(UInt32 /*inAttrIndex*/,
	...
	virtual void RemoveInstanceAttrComplete(UInt32 /*inAttrindex*/,
			QTSSDictionaryMap* /*inMap*/){}

	virtual QTSSDictionary* CreateNewDictionary(QTSSDictionaryMap* inMap,
			OSMutex* inMutex);
private:
	...
	struct DictValueElement
	{
		...
		DictValueElement() :
				fAllocatedLen(0), fNumAttributes(0), fAllocatedInternally(
...
public:
	...
	QTSS_Error GetAttrInfoByName(const char* inAttrName,
			QTSSAttrInfoDict** outAttrInfoDict,
			Bool16 returnRemovedAttr = false);
	...
	UInt32 GetNumAttrs()
	{
		return fNextAvailableID;
	}

	UInt32 GetNumNonRemovedAttrs()
	{
		return fNumValidAttrs;
	}

	Bool16 IsPreemptiveSafe(UInt32 inIndex)
	{
		Assert(inIndex < fNextAvailableID);
		return (Bool16) (fAttrArray[inIndex]->fAttrInfo.fAttrPermission
	...
	Bool16 InstanceAttrsAllowed()
	{
		return (Bool16) (fFlags & kInstanceAttrsAllowed);
	}

	Bool16 CompleteFunctionsAllowed()
	{
		return (Bool16) (fFlags & kCompleteFunctionsAllowed);
	}
...
inline SInt32 QTSSDictionaryMap::ConvertAttrIDToArrayIndex(
