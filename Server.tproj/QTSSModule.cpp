...
void QTSSModule::Initialize() {
	...
	for (UInt32 x = 0; x < qtssModNumParams; x++)
		QTSSDictionaryMap::GetMap(QTSSDictionaryMap::kModuleDictIndex)->SetAttribute(
...
QTSS_Error QTSSModule::SetupModule(QTSS_CallbacksPtr inCallbacks,
	...
	qtss_snprintf(msgStr, sizeof(msgStr), "Module Loaded...%s [%s]", moduleName,
...
QTSS_Error QTSSModule::LoadFromDisk(QTSS_MainEntryPointPtr* outEntrypoint) {
	...
	this->SetValue(qtssModName, 0, theFileName.Ptr, theFileName.Len,
			QTSSDictionary::kDontObeyReadOnly);
...
SInt32 QTSSModule::GetPrivateRoleIndex(QTSS_Role apiRole) {
	...
	case QTSS_Initialize_Role:
		return kInitializeRole;
	case QTSS_Shutdown_Role:
		return kShutdownRole;
	case QTSS_RTSPFilter_Role:
		return kRTSPFilterRole;
	case QTSS_RTSPRoute_Role:
		return kRTSPRouteRole;
	case QTSS_RTSPAuthenticate_Role:
		return kRTSPAthnRole;
	case QTSS_RTSPAuthorize_Role:
		return kRTSPAuthRole;
	case QTSS_RTSPPreProcessor_Role:
		return kRTSPPreProcessorRole;
	case QTSS_RTSPRequest_Role:
		return kRTSPRequestRole;
	case QTSS_RTSPPostProcessor_Role:
		return kRTSPPostProcessorRole;
	case QTSS_RTSPSessionClosing_Role:
		return kRTSPSessionClosingRole;
	case QTSS_RTPSendPackets_Role:
		return kRTPSendPacketsRole;
	case QTSS_ClientSessionClosing_Role:
		return kClientSessionClosingRole;
	case QTSS_RTCPProcess_Role:
		return kRTCPProcessRole;
	case QTSS_ErrorLog_Role:
		return kErrorLogRole;
	case QTSS_RereadPrefs_Role:
		return kRereadPrefsRole;
	case QTSS_OpenFile_Role:
		return kOpenFileRole;
	case QTSS_OpenFilePreProcess_Role:
		return kOpenFilePreProcessRole;
	case QTSS_AdviseFile_Role:
		return kAdviseFileRole;
	case QTSS_ReadFile_Role:
		return kReadFileRole;
	case QTSS_CloseFile_Role:
		return kCloseFileRole;
	case QTSS_RequestEventFile_Role:
		return kRequestEventFileRole;
	case QTSS_RTSPIncomingData_Role:
		return kRTSPIncomingDataRole;
	case QTSS_StateChange_Role:
		return kStateChangeRole;
...
SInt64 QTSSModule::Run() {
	...
	if (events & Task::kUpdateEvent) {  // force us to update to a new idle time
