...
OS_Error TCPListenerSocket::Initialize(UInt32 addr, UInt16 port)
{
	...
	if (0 == err)
		do
		{
			...
			if (err != 0)
				break; // don't assert this is just a port already in use.
...
void TCPListenerSocket::ProcessEvent(int /*eventBits*/)
{
	...
	if (osSocket == -1)
	{
		...
		if (acceptError == EMFILE || acceptError == ENFILE)
		{
			...
			qtss_snprintf(errStr, sizeof(errStr) - 1,
	...
	if (theTask == NULL) {    //this should be a disconnect. do an ioctl call?
		...
		int one = 1;
		int err = ::setsockopt(osSocket, IPPROTO_TCP, TCP_NODELAY, (char*) &one,
				sizeof(int));
		AssertV(err == 0, OSThread::GetErrno());

		err = ::setsockopt(osSocket, SOL_SOCKET, SO_KEEPALIVE, (char*) &one,
				sizeof(int));
		AssertV(err == 0, OSThread::GetErrno());

		int sndBufSize = 96L * 1024L;
		err = ::setsockopt(osSocket, SOL_SOCKET, SO_SNDBUF, (char*) &sndBufSize,
				sizeof(int));
		AssertV(err == 0, OSThread::GetErrno());
