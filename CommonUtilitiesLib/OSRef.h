...
public:
	...
	OSRef() :
			fObjectP(NULL), fRefCount(0), fNextHashEntry(NULL)
	{
#if DEBUG
		fInATable = false;
		fSwapCalled = false;
...
private:
	...
	OSRefKey(OSRef *elem) :
			fStringP(&elem->fString), fHashValue(elem->fHashValue)
	{
	}
...
public:
	...
	OSRefTable(UInt32 tableSize = kDefaultTableSize) :
			fTable(tableSize), fMutex()
	{
	}
	~OSRefTable()
	{
...
public:
	...
	OSRefReleaser(OSRefTable* inTable, OSRef* inRef) :
			fOSRefTable(inTable), fOSRef(inRef)
	{
	}
	~OSRefReleaser()
	{
		fOSRefTable->Release(fOSRef);
	}
