...
Task::Task() :
		fEvents(0), fUseThisThread(NULL), fDefaultThread(NULL), fWriteLock(
				false), fTimerHeapElem(), fTaskQueueElem(), pickerToUse(
...
void Task::Signal(EventFlags events)
{
	...
	EventFlags oldEvents = atomic_or(&fEvents, events);
	if ((!(oldEvents & kAlive)) && 
		(TaskThreadPool::sNumTaskThreads > 0)) {
		if (fDefaultThread != NULL && fUseThisThread == NULL)
			fUseThisThread = fDefaultThread;
		if (fUseThisThread != NULL) {
			// Task needs to be placed on a particular thread.
			...
			//find a thread to put this task on
			unsigned int theThreadIndex = atomic_add(
					(unsigned int *) pickerToUse, 1);
			...
			TaskThreadPool::sTaskThreadArray[theThreadIndex]->fTaskQueue.EnQueue(
					&fTaskQueueElem);
		}
	}
}
...
void Task::SetThreadPicker(unsigned int* picker)
{
	...
	if (TASK_DEBUG)
	{
		...
		{
			qtss_printf(
					"Task::SetThreadPicker sShortTaskThreadPicker for task=%s\n",
					fTaskName);
		}
		else if (&Task::sBlockingTaskThreadPicker == pickerToUse)
		{
		...
		{
			qtss_printf(
					"Task::SetThreadPicker ERROR unknown picker for task=%s\n",
					fTaskName);
		}
	}
}
...
Task* TaskThread::WaitForTask()
{
	while (true)
	{
		...
		{
			if (TASK_DEBUG)
			qtss_printf("TaskThread::WaitForTask found "
					"timer-task=%s thread %p fHeap.CurrentHeapSize(%"_U32BITARG_") taskElem = %p enclose=%p\n",
		...
		if (theElem != NULL)
		{
			if (TASK_DEBUG)
			qtss_printf("TaskThread::WaitForTask found "
					"signal-task=%s thread %p fTaskQueue.GetLength(%"_U32BITARG_") taskElem = %p enclose=%p\n",
	...
	while (true)
	{
		...
		while (!doneProcessingEvent)
		{
			...
			if (theTimeout < 0) {
				...
				//that if an event sneaks in right as the task is returning from Run()
				//(via Signal) that the Run function will be invoked again.
				doneProcessingEvent = compare_and_store(Task::kAlive, 0,
						&theTask->fEvents);
				if (doneProcessingEvent)
					theTask = NULL;
			} else {
...
Task* TaskThread::WaitForTask()
{
	while (true)
	{
		...
		//wait...
		OSQueueElem* theElem = fTaskQueue.DeQueueBlocking(this,
				(SInt32) theTimeout);
		if (theElem != NULL)
			return (Task*) theElem->GetEnclosingObject();
		//
