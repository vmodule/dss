...
int select_removeevent(int which)
{

	{
		...
		if (which == sMaxFDPos)
		{
			...
			while (!FD_ISSET(sMaxFDPos, &sReadSet)
					&& !FD_ISSET(sMaxFDPos, &sWriteSet) && (sMaxFDPos > 0))
			{
				sMaxFDPos--;
			}
		}
		...
		//close the fd
		UInt32 theIndex = 0;
		while ((sFDsToCloseArray[theIndex] != -1)
				&& (theIndex < sizeof(fd_set) * 8))
			theIndex++;
...
int select_waitevent(struct eventreq *req, void* /*onlyForMacOSX*/)
{
	...
	while (!selecthasdata()) {
		...
		sNumFDsBackFromSelect = ::select(sMaxFDPos + 1,
			&sReturnedReadSet,&sReturnedWriteSet, NULL, &tv);
