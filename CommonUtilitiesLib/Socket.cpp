...
Socket::Socket(Task *notifytask, UInt32 inSocketType) :
		EventContext(EventContext::kInvalidFileDesc, sEventThread), fState(
				inSocketType), fLocalAddrStrPtr(NULL), fLocalDNSStrPtr(NULL), fPortStr(
...
{
	int one = 1;
	int err = ::setsockopt(fFileDesc, SOL_SOCKET, SO_REUSEADDR, (char*) &one,
			sizeof(int));
	Assert(err == 0);
}
...
{
	int one = 1;
	int err = ::setsockopt(fFileDesc, IPPROTO_TCP, TCP_NODELAY, (char*) &one,
			sizeof(int));
	Assert(err == 0);
}
...
{
	int one = 1;
	int err = ::setsockopt(fFileDesc, SOL_SOCKET, SO_KEEPALIVE, (char*) &one,
			sizeof(int));
	Assert(err == 0);
}
...
void Socket::SetSocketBufSize(UInt32 inNewSize)
{
	...
	int err = ::setsockopt(fFileDesc, SOL_SOCKET, SO_SNDBUF, (char*) &bufSize,
			sizeof(int));
...
OS_Error Socket::SetSocketRcvBufSize(UInt32 inNewSize)
{
	...
	int bufSize = inNewSize;
	int err = ::setsockopt(fFileDesc, SOL_SOCKET, SO_RCVBUF, (char*) &bufSize,
			sizeof(int));
...
OS_Error Socket::Send(const char* inData, const UInt32 inLength,
...
OS_Error Socket::WriteV(const struct iovec* iov, const UInt32 numIOvecs,
		UInt32* outLenSent)
{
	Assert(iov != NULL);
